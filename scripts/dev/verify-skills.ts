/**
 * ANV-0190 — Wrap `anvil skill lint --json` for the local source tree.
 *
...
function main(): void {
  const args = ['node', BIN, 'skill', 'lint', '--json']
  ...
  if (!stdout) {
    const failure = {
      ok: false,
      error: `anvil skill lint exited ${result.status ?? 1} with no output`,
    }
