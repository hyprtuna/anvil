/**
 * ANV-0193 — agent-e2e for ultra-worker after user-choice pattern rewrite.
 *
 ...
 *   4. The addendum file exists at agents/_addenda/ultra-worker-anvil.md and
 *      contains the extracted Anvil-specific gate content.
 *   5. The addendum contains ANVIL_SPECS_DIR, brainstorm-spec, <decisions>,
 *      anvil plan-validate-coverage — confirming the extraction was complete.
 *   6. Anvil flavor path: response must reference the SDD gate.
 *   7. Generic flavor path: response must NOT reference ANVIL_SPECS_DIR or
 *      brainstorm-spec.
 */
...
describe('agent-e2e: ultra-worker — user-choice pattern', () => {
  // ── 1. Agent body cleanliness ─────────────────────────────────────────────

  it('agent body greps clean for ${ANVIL_SPECS_DIR} token', async () => {
    const body = await loadAgentBody({ relativePath: 'ultra-worker.md' })
  ...
  it('agent body greps clean for --no-require-spec flag references', async () => {
    const body = await loadAgentBody({ relativePath: 'ultra-worker.md' })
    expect(body).not.toMatch(/--no-require-spec/)
  })
  ...
  it('addendum file exists at agents/_addenda/ultra-worker-anvil.md', async () => {
    const addendumPath = resolve(AGENTS_ROOT, '_addenda/ultra-worker-anvil.md')
  ...
  it('Generic flavor path: body text does not contain SDD gate instructions inline', async () => {
    const body = await loadAgentBody({ relativePath: 'ultra-worker.md' })
    // The SDD gate logic must be absent from the main body (it's in the addendum)
    expect(body).not.toMatch(/Spec hard.gate/i)
    expect(body).not.toMatch(/ANVIL_SPECS_DIR/)
    expect(body).not.toMatch(/brainstorm-spec/)
  })

  it('Anvil flavor: addendum describes how to locate spec file when --require-spec is active', async () => {
    const addendumPath = resolve(AGENTS_ROOT, '_addenda/ultra-worker-anvil.md')
    const raw = await readFile(addendumPath, 'utf-8')
    // Addendum should describe locating the spec
    expect(raw).toMatch(/spec file|locate.*spec|find.*spec/i)
  })
})
