...
describe.skip('version bump — v0.12.0', () => {
  it('package.json version is 0.12.0', () => {
    const raw = readFileSync(PACKAGE_JSON_PATH, 'utf8')
    const pkg = JSON.parse(raw) as { version: string }
    expect(pkg.version).toBe('0.12.0')
  })

  it('marketplace.json version is 0.12.0', () => {
    const raw = readFileSync(MARKETPLACE_JSON_PATH, 'utf8')
    const mkt = JSON.parse(raw) as { version: string }
    expect(mkt.version).toBe('0.12.0')
  })

  it('package.json version is not stale 0.11.2', () => {
    const raw = readFileSync(PACKAGE_JSON_PATH, 'utf8')
    const pkg = JSON.parse(raw) as { version: string }
    expect(pkg.version).not.toBe('0.11.2')
  })

  it('marketplace.json version is not stale 0.11.2', () => {
    const raw = readFileSync(MARKETPLACE_JSON_PATH, 'utf8')
    const mkt = JSON.parse(raw) as { version: string }
    expect(mkt.version).not.toBe('0.11.2')
  })
